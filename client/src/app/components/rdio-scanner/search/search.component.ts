...
import {BehaviorSubject, debounceTime, distinctUntilChanged, filter, fromEvent, tap} from 'rxjs';
import {
  RdioScannerCall,
  RdioScannerConfig,
  RdioScannerEvent,
...
export class RdioScannerSearchComponent implements OnDestroy, OnInit {
  ...
  @ViewChild('keywordInput', {static: true}) keywordInput: ElementRef | undefined;
  ...
  ngOnInit() {
    fromEvent<any>(this.keywordInput?.nativeElement,'keyup')
      .pipe(
        filter(Boolean),
        debounceTime(500),
